...
- `/playlist url:str channel:VoiceChannel(Optional) shuffle:Bool(Optional)` 使用者可以選擇一個語音頻道，將一個Youtube播放清單加入列隊並播放
  - 若列隊已有撥放項目，則將播放清單內容依序加入列隊中
  - 如果`shuffle = True`則播放清單會先被打亂再開始播放
- `/shuffle` 將目前列隊打亂
  - **打亂列隊為不可逆之行為**
- `/queue` 取得目前列隊內容
  - 如果對列超過25首歌，將會分段顯示
